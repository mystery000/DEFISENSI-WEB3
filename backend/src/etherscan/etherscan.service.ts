...
export class EtherscanService {
  ...
  async updateTokenTransactions(address: string, network: string, startBlockNumber: number) {
    ...
    if (toBlockNumber - startBlockNumber > 1000) toBlockNumber = startBlockNumber + 1000;
  ...
  async getTransactionDetail(txhash: string, address: string, network: string = NetworkType.Ethereum) {
    ...
    const created = Number(blockData.timestamp);
    const coingeckoDate = convertToCoinGeckoDate(created * 1000);
    ...
    if (isSwap) {
      ...
      const price0 = await this.getPrice(symbol0, coingeckoDate);
      ...
      const price1 = await this.getPrice(symbol1, coingeckoDate);
      ...
      if (extractedFunctionSelector === transferFunctionSelector) {
        ...
        const price = await this.getPrice(symbol, coingeckoDate);
        ...
        if (Number(transactionDetails.value) > 0 && transactionDetails.input === '0x') {
          const price = await this.getPrice('eth', coingeckoDate);
          return {
