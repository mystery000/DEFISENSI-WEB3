...
export const WalletNotification: FC<WalletNotificationProps> = ({ notification, setNotifications }) => {
    ...
    <div className="w-[382px] rounded-md bg-white p-[20px]">
      ...
      <div className="my-3 flex flex-col gap-3">
        <div className="flex items-center gap-12">
          <div className="w-1/3 text-sm text-bali-hai-600">Wallets ({notification?.subscribeTo?.length || 0})</div>
          <span className="font-sora font-semibold">
            {notification.subscribeTo &&
              notification.subscribeTo.map((address) => convertHex(address).slice(0, 5)).join(',')}
          </span>
        </div>
        {notification?.maxUsd && (
          <div className="flex items-center gap-12">
        ...
        <div className="flex items-center gap-12">
          ...
          <div className="flex items-center">
            <img src={`/images/network/ethereum.png`} width={20} height={20} alt="noIcon" className="rounded-full" />
            <span className="font-sora font-semibold">{notification?.maxTokenValue?.toLocaleString()}</span>
          </div>
        </div>
        <div className="flex items-center gap-12">
          ...
          <div className="flex items-center gap-2">
            {notification.tokens?.map((token) => (
              <img src={`/images/network/${token}.png`} width={20} height={20} alt="noIcon" className="rounded-full" />
            ))}
          </div>
        </div>
        <div className="flex items-center gap-12">
          ...
          <div className="flex items-center -space-x-1.5">
            {notification.network.map((chain) => (
              <Image src={`/images/network/${chain}.png`} width={20} height={20} alt="#" className="rounded-full" />
            ))}
      ...
      <Modal
        ...
        width={1024}
      >
        <WalletNotificationPage data={notification} handleEditAlert={handleEditAlert} />
      </Modal>
    </div>
  );
};
