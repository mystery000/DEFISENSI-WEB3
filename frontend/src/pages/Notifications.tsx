...
import { NFTAlertIcon, TokenAlertIcon, WalletAlertIcon } from '../components/icons/defisensi-icons';
...
export const Notifications = () => {
  ...
  if (error) {
    return (
      <div className="grid h-screen place-items-center">
        <span className="font-sora text-2xl font-semibold">Network connection error</span>
      </div>
    );
  }
  ...
  return (
    <AppLayout>
      <div className="w-full font-inter xl:mx-auto xl:w-2/3">
        ...
        >
          <div>
            <div className="font-sora text-[32px] font-semibold">Notifications</div>
            <div className="text-sm">How do you want us to send you notifications?</div>
          </div>
            ...
            {notifications.length ? (
              notifications.map((notification) =>
                notification.type === NotificationType.WALLET ? (
                  <WalletNotification notification={notification} setNotifications={mutate} key={notification._id} />
                ) : notification.type === NotificationType.TOKEN ? (
                  <TokenNotification notification={notification} setNotifications={mutate} key={notification._id} />
                ) : (
                  <NFTNotification notification={notification} setNotifications={mutate} key={notification._id} />
                ),
              )
            ) : (
      ...
      <Modal
        ...
        width={350}
      >
        <p className="my-2 font-sora text-2xl font-semibold">Create New Alert</p>
        <div
        ...
        >
          ...
          <div>
            ...
            <div className="font-sora text-[10px] text-bali-hai-600">Create New Alert for Wallet</div>
          </div>
        </div>
        <div
          className="my-2 flex items-center gap-6 rounded-md border border-bali-hai-600/20 px-4 py-2 transition duration-300 hover:border-royal-blue-500"
        ...
        >
          ...
          <div>
            ...
            <div className="font-sora text-[10px] text-bali-hai-600">Create New Alert for Token</div>
          </div>
        </div>
        <div
          className="my-2 flex items-center gap-6 rounded-md border border-bali-hai-600/20 px-4 py-2 transition duration-300 hover:border-royal-blue-500"
        ...
        >
          ...
          <div>
            ...
            <div className="font-sora text-[10px] text-bali-hai-600">Create New Alert for NFT</div>
          </div>
        </div>
      </Modal>
    </AppLayout>
